...
where X, U and W correspond to the labels used in Table 3-4 UTF-16 Bit Distribution. The next snippet does the same for the low surrogate.
...
//CGS MOD (ADDED FUNCTION FOR USING IT WITH THE OSK INPUT)
int cccUCS2toUTF8(cccCode * dst, size_t count, cccUCS2 * str) {
	if (!str || !dst) return 0;

    int i = 0, length = 0;
    while (str[i] && length < count) {
		if  (str[i] <= 0x7FU) {       //ASCII
			dst[length] = (cccCode)str[i]; 
			i++;    length++; 
		} else if (str[i] <= 0xC1U) { //part of multi-byte or overlong encoding ->ignore
			i++;          
		} else if (str[i] <= 0xDFU) { //2-byte
