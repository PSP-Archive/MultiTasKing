...
//    intext = (unsigned short *) malloc((textLimit + 1)*sizeof(unsigned short));	//<-- STAS: textLimit ISN'T right!
	int initialStrLen = strlen(initialStr);
    intext = (unsigned short *) malloc((initialStrLen + 1)*sizeof(unsigned short));
    if (!intext)
        return;
	
	int descStrLen = strlen(descStr);
    desc = (unsigned short *) malloc((descStrLen + 1)*sizeof(unsigned short));
    if (!desc) {
        oslEndOsk();
        return;
    }
    
    cccUTF8toUCS2(intext, initialStrLen, (unsigned char*)initialStr);
    
    cccUTF8toUCS2(desc, descStrLen, (unsigned char*)descStr);
...
//		case PSP_UTILITY_DIALOG_INIT:			//<-- STAS: sceUtilityOskUpdate should be called only in VISIBLE state!
        case PSP_UTILITY_DIALOG_VISIBLE :
            //sceDisplayWaitVblankStart();//CGS MOD
            //sceDisplayWaitVblankStart();//CGS MOD
			sceGuFinish();
			sceGuSync(0,0);
            sceUtilityOskUpdate(1/*CGS MOD, it was 2*/);//CGS MOD - NOTE: with 1 the keyboard can work at 120 FPS, with 2 we need to refresh this function at 60 FPS or the keyboard typing will speed up a lot
...
unsigned short* oslOskGetOutData(){//CGS MOD
    return osl_osk->oskParams.data[0].outtext;
}

char* oslOskGetFullTextChar(){//CGS MOD
    return (char*)osl_osk->oskParams.data[0].outtext;
}
