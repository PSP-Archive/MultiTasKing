...
void MTloadFailCallback(const char *filename, u32 reserved) {
	...
	MTapiWindow* window = MTapiCreateWindow("Error when loading file", -1, 50, drawMyWin, RGBA(255,255,255,255));
	
	window->cursorType = 14;
	//window->data = (char*)MTmalloc(256 * sizeof(char));
	asprintf(window->data,"This file could not be loaded:\n\"%s\"",filename);
	//a
...
int main(int argc, char** argv) {
	...
	if(MTdoesFileExists("Data.zip") == 1) {
		...
		while(extraction.done == 0 && extraction.error == 0) {
			if(!skip) {
				...
				sprintf(text,"Please wait while MultiTasKing starts for the first time...\nThis can take a few minutes...\n\n%d%% completed.",extraction.percent);
		...
		if(extraction.ret == 0 && extraction.error == 0) {
			...
			MTclearScreen (MTapiWindowBarInColor);
			MTdrawSmoothFillRoundRectangle(0, 0, screenWidth, screenHeight, MTapiWindowBarOutColor, 20, 4);
			MTdrawString(30,30,"There was an error while extracting system files.\nPlease, reinstall MultiTasKing.\nReturning to XMB in 10 seconds...");
			MTendDrawing();
			MTsyncDrawing();
			MTdelay(10000000);
			MTendMainLib();
		...
		MTclearScreen (MTapiWindowBarInColor);
		MTdrawSmoothFillRoundRectangle(0, 0, screenWidth, screenHeight, MTapiWindowBarOutColor, 20, 4);
		MTdrawString(30,30,"The file \"Data.zip\" was not found.\nPlease, reinstall MultiTasKing.\nReturning to XMB in 10 seconds...");
		MTendDrawing();
		MTsyncDrawing();
		MTdelay(10000000);
		MTendMainLib();
	...
	if(MTcanUseKernel == 0) {
		MTshowErrorWindow("Kernel mode disabled", "You are executing MultiTasKing in\nkernel mode, and the following systems won't work:\n - MP3 and AT3 sounds\n - Network functions\n - Eboot loading");
		//return NULL;
	}
