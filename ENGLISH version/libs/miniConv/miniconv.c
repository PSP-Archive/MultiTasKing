...
unsigned char* utf16le_to_utf8_alloc(unsigned short* str, int* size) {
    ...
    while (i < len) {
		...
		if ( ret_val < 0 ) { 
			i++;
			dst = (unsigned char*) realloc(dst,(j + 1) * sizeof(unsigned char*));
			dst[j] = '?';
			j++;
		}
		else {
			i += ret_val;
			dst = (unsigned char*) realloc(dst,(j + 6 + 1) * sizeof(unsigned char*));
			ret_val = utf8_wctomb((unsigned char*)dst+j, ucs4_char, 6);
			if ( ret_val > 0 ) j += ret_val;
		}
	}
...
char* utf16le_to_utf8_old(const unsigned short* utf16le) {
	...
	while( i < ilen) {
		...
		if ( ret_val < 0 ) { 
			...
			i += ret_val;
			ret_val = utf8_wctomb((unsigned char*)convert_buffer+j, ucs4_char, 6);
			if ( ret_val > 0 )
				j += ret_val;
		}
	}
