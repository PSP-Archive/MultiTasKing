...
void MTlaunchAppDownPSPfileInstaller(char* url) {
	if(url == NULL) return;
	if(MTappDataDownPSPloginSuccess == 0) MTshowErrorWindow("Error: You have not logged into SceneBeta", "You have not logged into SceneBeta\nand it is not possible to continue.");
		...
		if(data->state == 0) {
			...
			if(data->download->done && data->download->error == 0) {
				...
				if(data->download->error) {
					MTdrawString(10,10,"Error while downloading");
					window->cursorType = 14;
					data->state = 0;
					MTshowErrorWindow("Error while downloading", "There was an error, and\nit was not possible to continue.");
				} else {
					if(data->download->knownsize) {
					char buff[256];
					sprintf(buff,"Downloading homebrew...\n%d%% completed.",data->download->percent);
					MTdrawString(10,10,buff);
					} else MTdrawString(10,10,"Downloading homebrew...");
					window->cursorType = 2;
				}
			}
			...
			if(data->extraction->done && data->extraction->error == 0) {
				...
				if(data->extraction->error) {
					MTdrawString(10,10,"Error when uncompressing");
					window->cursorType = 14;
					data->state = 0;
					...
					data->extraction = NULL;
					MTshowErrorWindow("Error when uncompressing", "There was an error, and\nit was not possible to continue.");
				} else {
					MTdrawString(10,10,"Uncompressing and installing...");
					window->cursorType = 2;
				}
			}
		} else if(data->state == 3) {
			MTdrawString(10,10,"The homebrew has been installed correctly...");
			window->cursorType = 0;
		}
	}
	...
	asprintf(&data->data,"Press X to install:\n%s",data->path);
	//MTfree(name);
	
	//MTdownload* download
	//MTaddToDownloadQueue(data->download);
	
	MTapiWindow* window = MTapiCreateWindow("downPSP - installer", 260, 50, &drawMyWin, RGBA(255,255,255,255));
	...
	void drawMyWin(MTapiWindow* window) {
		...
		if(data->state == 0) {
			if(data->download->done && data->download->error == 0 && data->download->file != NULL) {
				...
				if(data->download->error) {
					MTdrawString(10,10,"Error when downloading");
					window->cursorType = 14;
				} else {
					if(data->download->knownsize) {
					char buff[256];
					sprintf(buff,"Receiving...\n%d%% completed.",data->download->percent);
					MTdrawString(10,10,buff);
					} else MTdrawString(10,10,"Receiving...");
					window->cursorType = 2;
				}
			}
		} else if(data->state == 1) {
			...
			if(MTisOverAreaSimple(window->mouse.x, window->mouse.y, 0, 0, window->size.x - 15, window->size.y) && window->tapControls.cross) {
				...
				data->state = 2;
				data->keyboard = MTcreateKeyboard("User", "scenebeta.com username", 256);
			...
			window->cursorType = 2;
			MTdrawString(10,10,"Logging in www.scenebeta.com...\nType in your username");
			if(data->keyboard->done) {
				if(data->keyboard->cancel == 0) {
					...
					data->keyboard = NULL;
					data->keyboard = MTcreateKeyboard("Password", "scenebeta.com password", 256);
					MTaddToKeyboardQueue(data->keyboard);
					data->state = 3;
				} else {
					data->state = 1;
			...
			if(data->download->done && data->download->error == 0 && data->download->file != NULL) {
				...
				if(strstr(data->download->file, "Inicio de sesi") != NULL) {
					data->state = 1;
					MTshowErrorWindow("Error while logging in", "It was not possible to log in\nwith the specified data.");
					MTfreeDownload(data->download);
				...
				if(data->download->error) {
					...
					if(data->download->knownsize) {
					char buff[256];
					sprintf(buff,"Logging in www.scenebeta.com...\nType in your username.\nType in your password.\nLogging in...\nReceiving...\n%d%% completed.",data->download->percent);
					MTdrawString(10,10,buff);
					} else MTdrawString(10,10,"Logging in www.scenebeta.com...\nType in your username.\nType in your password.\nLogging in...\nReceiving...");
					window->cursorType = 2;
				}
			}
			//window->cursorType = 2;
			...
			if(data->download->done && data->download->error == 0 && data->download->file != NULL) {
				...
				if(attachments) {
					...
					while(1) {
						...
						if(attachments) {
							...
							if(foundAny == 0) MTshowErrorWindow("No download was found", "This homebrew doesn't have\nany valid download.");
						   break;
						}
					}

				} else {
					MTshowErrorWindow("No download was found", "This homebrew doesn't have\nany valid download.");
				}
				MTfreeDownload(data->download);
				...
				if(data->download->error) {
					...
					if(data->download->knownsize) {
					char buff[256];
					sprintf(buff,"Receiving download list...\n%d%% completed.",data->download->percent);
					MTdrawString(10,10,buff);
					} else MTdrawString(10,10,"Receiving download list...");
					window->cursorType = 2;
				}
			}
...
void MTlaunchAppDownPSP() {
	...
	void drawMyWinDownloading(MTapiWindow* window) {
		...
		if(download->done && download->error == 0 && download->file != NULL) {
			...
			//} else {
				window->cursorType = 0;
				window->data = MTapiCreateMenu(0, 0, window->size.x, window->size.y, "All programs","   - ");
			...
			if(download->error) {
				MTdrawString(10,10,"Error while downloading list");
			} else {
				if(download->knownsize) {
				char buff[256];
				sprintf(buff,"Downloading homebrew list...\n%d%% completed.",download->percent);
				MTdrawString(10,10,buff);
				} else MTdrawString(10,10,"Downloading homebrew list...");
			}
		}
	}
