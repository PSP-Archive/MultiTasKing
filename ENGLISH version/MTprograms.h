...
void MTlaunchAppMultiTasKingCredits() {
	...
	MTapiWindow* window = MTapiCreateWindow("MultiTasKing credits", -1, 150, &drawMyWin, RGBA(255,255,255,255));
	
	window->cursorType = 0;
	window->data = MTapiCreateTextBox(0, 0, window->size.x, window->size.y, "MultiTasKing v300\nDeveloped by Carlosgs\nhttp://www.carlosgs.es\n\nIf you have liked it, please vote for\nMultiTasKing in the SceneryBeta 2010.\n\n\nGreetings:\n\nDeViaNTe (http://www.gcrew.es)\nhttp://psp.scenebeta.com\nNiteman\nUnicorn\nArber\n\n\nBetatesters:\n\nRubenelportero\nDark-Santi\nQarl\nAlbert\nArber\nDeviante\n\n\nResources:\n\nOSlib mod 1.0.1\nTango icon pack\nVanilla DMZ cursors\n\nLook inside the MultiTasKing folders to see more information about copyright.\n\n");
...
void MTlaunchAppTextViewer(char* name, char* path) {
	...
	if(file == NULL || size <= 0) asprintf(&file,"This file is empty.");
...
void MTlaunchAppFileBrowser(char* iniPath) {
	...
	MTapiWindow* window = MTapiCreateWindow("File browser", 220, 130, &drawMyWin, RGBA(255,255,255,255));
...
void MTlaunchAppBackgroundSelector() {
	...
	MTapiWindow* window = MTapiCreateWindow("Select the background image", 220, 130, &drawMyWin, RGBA(255,255,255,255));
...
void MTlaunchAppUSBconnect() {
	...
	MTapiWindow* window = MTapiCreateWindow("USB connection", -1, 50, &drawMyWin, RGBA(255,255,255,255));
...
void MTlaunchAppGeolocator() {
	...
	void drawMyWinDownloading(MTapiWindow* window) {
		...
		if(download->done && download->error == 0 && download->file != NULL) {
			...
			if(download->error) {
				MTdrawString(10,10,"Error while downloading location information.");
			} else {
				if(download->knownsize) {
				char buff[256];
				sprintf(buff,"Downloading location information...\n%d%% completed.",download->percent);
				MTdrawString(10,10,buff);
				} else MTdrawString(10,10,"Downloading location information...");
			}
		}
	}
	...
	MTapiWindow* window = MTapiCreateWindow("Geolocation", 240, 120, &drawMyWinDownloading, RGBA(255,255,255,255));
...
void MTlaunchAppLauncher() {
	...
	MTapiWindow* window = MTapiCreateWindow("Main menu", 250, 100, &drawMyWin, RGBA(255,255,255,255));
	...
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("File browser", NULL, MTappLauncherFileBrowser),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("Scenery Beta 2010 - Presentation", NULL, MTappLauncherSceneryBetaAnimation),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("mini downPSP", NULL, MTappLauncherDownPSP),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("Geolocator", NULL, MTappLauncherGeolocator),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("Select the background image", NULL, MTappLauncherBackgroundSelector),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("USB connection", NULL, MTappLauncherUSBconnect),0);
	MTapiAddMenuElement(window->data,MTapiCreateMenuElementWithFunction("MultiTasKing credits", NULL, MTlaunchAppMultiTasKingCredits),0);
